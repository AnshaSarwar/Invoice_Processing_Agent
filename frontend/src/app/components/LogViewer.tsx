      ...
      <>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {logs.slice(0, 12).map((log) => (
            <div
              key={log.id}
              className="glass-card p-5 group transition-all border border-white/5 hover:border-blue-500/30 cursor-pointer"
            ...
            >
              <div className="flex justify-between items-start mb-3">
                ...
                {canUserDelete(log) && (
                  <button
                    ...
                    className="opacity-40 hover:opacity-100 text-red-400 transition-all text-sm p-1 hover:bg-red-500/10 rounded-md"
              ...
              {log.error_message && log.status !== 'completed' && (
                <p className="text-[10px] text-red-400/80 truncate mt-1">{log.error_message}</p>
              )}
              {log.uploader_name && (isAdmin || isManager) && (
                <div className="mt-2 flex items-center gap-1.5 opacity-60">
                  <span className="text-[10px] grayscale">👤</span>
                  <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-tighter">Uploaded by: {log.uploader_name}</span>
                </div>
              )}
      ...
      <div className="glass rounded-xl overflow-hidden border border-white/10">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-[#18181b] text-zinc-500 uppercase font-bold tracking-tighter">
              <tr>
              ...
              {logs.map((log) => (
                ...
                >
                  <td className="px-6 py-4">
                    <span className={`text-[10px] font-bold ${log.status === 'completed' ? 'text-green-400' : 'text-red-400'}`}>
                      ● {log.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-medium text-zinc-200">{log.filepath?.split(/[\/\\]/).pop()}</td>
                  <td className="px-6 py-4 font-mono text-blue-300">{log.po_number || '—'}</td>
                  <td className="px-6 py-4">
                    <span className="text-blue-400 font-bold">
                      {log.confidence_score != null ? `${(log.confidence_score * 100).toFixed(0)}%` : '—'}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-zinc-400">{log.processing_time_seconds?.toFixed(1) ?? '—'}s</td>
